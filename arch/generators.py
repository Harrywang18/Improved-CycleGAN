...
from .ops import conv_norm_relu, dconv_norm_relu, ResidualBlock, get_norm_layer, init_network, _DenseBlock, _Transition
from .ops import DenseNet
import torchvision
...
class DenseGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True,
                 num_blocks=6, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64,
       ...
       # res_model += [nn.Conv2d(128, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)]
       # res_model += [nn.BatchNorm2d(128)]
       # res_model += [nn.ReLU(inplace=True)]
       # res_model += [nn.MaxPool2d(3, stride=2, padding=1)]

        num_features = 128
        #res_model += [DenseNet(growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64,
                              # bn_size=4, compression_rate=0.5, drop_rate=0, num_classes=1000)]
        for i, num_layers in enumerate(block_config):
            block = _DenseBlock(num_layers, 128, bn_size, growth_rate, drop_rate)
            res_model += [_DenseBlock(num_layers, num_features, bn_size, growth_rate, drop_rate)]
            num_features += num_layers * growth_rate
            #if i != len(block_config) - 1:
                #transition = _Transition(num_features, int(num_features * compression_rate))
                #res_model += [transition]
                #num_features = int(num_features * compression_rate)

        res_model += [
                      nn.Conv2d(1984, 128, 1, 1, 0),
...
def define_Gen(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, gpu_ids=[0]):
    ...
    if netG == 'densenet':
        gen_net = DenseGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, num_blocks=9)
